...
def plot_rewards(df, filename=None):
    ...
    rewards_plot_data = [
        go.Scatter(x=df.index, y=df[col], name=col) for col in reward_columns
    ]

    layout = go.Layout(
        title="Cumulative Average reward recieved over time",
...
def dist_plots(
    dfs, labels, title, bin_size=0.1, show_curve=True, show_hist=True, filename=None
):
    # Create distplot with custom bin_size
    fig = ff.create_distplot(
        dfs, labels, bin_size=bin_size, show_curve=show_curve, show_hist=show_hist
    )
